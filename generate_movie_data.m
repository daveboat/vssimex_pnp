% Let's visualize the problem --- vary dt and look at the magnitude of the
% eigenvalues...
clear
...
clear cp cm phi_x_right phi t err_save
n=1;
cp(:,n) = cp_init;
cm(:,n) = cm_init;
...
for jj=length(EPSILON)-1:-1:1
    ...
    clear cp cm phi phi_x_right debug err_save t
...
for j=length(EPSILON):-1:1
    ...
    test = 'quadratic';
    h = .01;
    [B_new,B_now,B_old] = auto_linearize_RHS(h,test,v(j),v(j),dt,dt,phi_x_right,phi_x_right,cp,cp,cp,cm,cm,cm,bc, dx,N1,N2,N3, lambda_s,D_0,D_p,D_m,kc_left,jr_left,kc_right,jr_right,epsilon_1,epsilon_2,epsilon_3,epsilon_s_left,epsilon_s_right,z_cp,z_cm,k,a_left,a_1,a_2,a_3,a_right,h_left,h_right,gamma_left,gamma_right,nu,frum );
   
%     dt = (2*dt_end(j)-.1*dt_end(j))/100;
%     DT = .1*dt_end(j):dt:2*dt_end(j);
    
    for ii=length(DT):-1:1
        dt_now = DT(ii);
        ...
        d(I,I) = 0; % set the eigenvalue of magnitude 1 to zero in the diagonal matrix; this will be needed for when I start trying to find the numerical crossing
        a = diag(d); % now extract the diagonal
        [aa,I] = sort(abs(a));
        EIGS(j,:,ii) = a(I); % save all eigenvalues except the magnitude 1 eigenvalue
    end
%     figure(3)
%     clf
%     hold on
...
%     %axis([min(DT),max(DT),.6,1.05])
%     % axis([0,.0005,.6,1.2])
%     TITLE = strcat('all eigenvalues, epsilon = ', num2str(epsilon), ', dt_{thresh} = ',num2str(dt_thresh(j)));
%     title(TITLE)
%     figure(3)
%     pause(1)
end
