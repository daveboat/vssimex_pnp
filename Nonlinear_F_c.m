function NF = Nonlinear_F_c(dt_now,dt_old,phi,cp,cp_now,cp_old,cm,cm_now,cm_old,j_m_left,j_m_right,j_p_left,j_p_right, dx,N1,N2,N3,D_0,D_p,D_m,z_cp,z_cm)
...
cm_LHS = (((1+2*w)/(1+w))*cm - (1+w)*cm_now + (w^2/(1+w))*cm_old)/dt_now;
cp_LHS = (((1+2*w)/(1+w))*cp - (1+w)*cp_now + (w^2/(1+w))*cp_old)/dt_now;
